...
TEXT_PADDING_VALUE = 151_860
UNIT_PADDING_VALUE = 10_000

DATASET = "commonvoice"
if DATASET == "commonvoice":
    DATA_FOLDER = "/scr-ssd/duyy/qwen2_commonvoice_TUpair"
elif DATASET == "mls":
...
def train(model, device, train_loader, val_loader, optimizer, epoch, rank, run_name, with_prior, validate_every_n_batches=250):
    ...
    if rank == 0:
        print("Saving...")
        SAVE_PATH = f"/sailhome/duyy/data/checkpoints/ForcedAligner-qwen/ckpt-{run_name}/"
...
def main():
    ...
    # CONTINUE_TRAINING = "/sailhome/duyy/data/checkpoints/ForcedAligner-qwen/ckpt-2024-09-27_05-40-52-commonvoice-NGPU-1_BS-64_LR-5e-05-EPOCH-10-with-prior/epoch-3.bin" #"/afs/cs.stanford.edu/u/duyy/data/checkpoints/ForcedAligner/ckpt-2024-07-25_00-02-53-NGPU-4_BS-64_LR-5e-05-EPOCH-5-continue/epoch-4.bin"
    CONTINUE_TRAINING = "/sailhome/duyy/data/checkpoints/ForcedAligner-qwen/ckpt-2024-09-29_06-38-17-commonvoice-NGPU-2_BS-64_LR-5e-05-EPOCH-6-with-prior-continue/epoch-5.bin"
    ...
    train_text_tokens, train_unit_tokens = load_data(data_folder, range(24)) # 24 for commonvoice and 4095 for mls
    # import ipdb; ipdb.set_trace();
    val_text_tokens, val_unit_tokens = load_data(data_folder, [23])
    ...
    model = Aligner(TEXT_PADDING_VALUE+1, UNIT_PADDING_VALUE+1)
...
def inference(x = 0, with_prior=False):
    ...
    output_path = f"/scr-ssd/duyy/Alignment-qwen/result/chunk_{x}.pkl"

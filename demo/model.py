...
ENCODER_TYPE = "llama3"
assert ENCODER_TYPE in ["llama3", "llama3_embed"]
...
class LlamaDecoder(nn.Module):
    ...
    def forward(self, src, **kwargs):
        ...
        # Debug
        cur_time = time.time()
        print("Encoder time elapse:", cur_time - prev_time)
        prev_time = cur_time
        ...
        # Debug
        cur_time = time.time()
        print("Unit decoder time elapse:", cur_time - prev_time)
...
def greedy_decode(model, src, src_mask, max_len, start_symbol, sep_token, answer=None, gt_align=None):
    ...
    for i in range(len(src[0])):
        ...
        for j in range(max_len-1):
            ...
            # yield
            ys_len = ys.shape[-1]
            if (ys_len - current_pos) >= 50:
                yield ys[..., current_pos:]
                current_pos = ys_len
...
def unit_to_audio(vocoder_agent, unit_tokens):
...
def load_model(model_path):
    ...
    if ENCODER_TYPE == "llama3":
        encoder = LlamaDecoder(ckpt_dir, tokenizer_path, max_seq_len, max_batch_size, EMBED_DIM, False, device) 
    elif ENCODER_TYPE == "llama3_embed":
        encoder = LlamaDecoder(ckpt_dir, tokenizer_path, max_seq_len, max_batch_size, EMBED_DIM, True, device) 
    ...
    return model
def load_vocoder_agent(vocoder_args=None):
    if vocoder_args is None:

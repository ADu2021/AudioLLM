...
MEMORY_MASK_TYPE = "causal" # independent or causal
...
NUM_UNIT_TOKENS = NORMAL_UNIT_TOKENS + 3  # Example token size including <eos> and <sos> tokens
...
def greedy_decode(model, src, src_mask, max_len, start_symbol, sep_token, answer, gt_align):
    ...
    for i in range(len(src[0])):
        ...
        for j in range(max_len-1):
            ...
            if gt_align[0][-1] == (alignment_lens[0][-1] + 1):
                break
...
def demo(text_tokens, unit_tokens, model_name, id):
    ...
    non_sep_mask = unit_tokens < NORMAL_UNIT_TOKENS
...
def main_inference():
    ...
    # known-alignment
    # model_path = "/afs/cs.stanford.edu/u/duyy/data/checkpoints/TTU/ckpt-2024-08-05_16-43-05-LLaMA-KnownAlignment-NGPU-2_BS-64_LR-1e-05-Warmup-6000-EPOCH-4-EMBED-1024-FFN-2048-DR-0.1-NH-8-NL-6-causal_tgt_mask-independent_mem_mask/epoch-3.bin"
    model_path = "/afs/cs.stanford.edu/u/duyy/data/checkpoints/TTU/ckpt-2024-08-05_16-43-50-LLaMA-KnownAlignment-NGPU-2_BS-64_LR-1e-05-Warmup-6000-EPOCH-4-EMBED-1024-FFN-2048-DR-0.1-NH-8-NL-6-causal_tgt_mask-causal_mem_mask/epoch-3.bin"
    ...
    text_tokens, unit_tokens, alignment = load_data([0])
    print("data done.")
    ...
    for i in range(0, 50):
        ...
        start_symbol = BOS_TOKEN # We use sep token as start symbol
        sep_token = EOS_TOKEN # not used

...
# TrainingConfig
LR = 1e-5
WARMUP_STEPS = 3000 # 2000 for 4gpu
BATCH_SIZE = 32
EPOCHS = 5
NUM_CYCLES = 2
WANDB = True
...
UNIT_EMBED_PATH = None #"/afs/cs.stanford.edu/u/duyy/.cache/fairseq2/assets/aa367aa51efaa58032e42d54/kmeans_10k.npy"
if UNIT_EMBED_PATH is not None:
    assert EMBED_DIM == 1280

CONTINUE_TRAINING_PATH = "/afs/cs.stanford.edu/u/duyy/data/checkpoints/TTU/ckpt-2024-08-08_22-18-05-LLaMA-UnitEmbed-Seperated-Resample-NGPU-3_BS-64_LR-1e-05-Warmup-3000-EPOCH-6-EMBED-1280-FFN-2048-DR-0.1-NH-8-NL-6-causal_tgt_mask-causal_mem_mask/epoch-4.bin"
...
def main():
    ...
    run_name = f"{current_time}-LLaMA{if_unit_embed}{if_continue}{if_only_first_seperated}{if_resample}{weight_sep}-NGPU-{GPU_COUNT}_BS-{BATCH_SIZE}_LR-{LR}-Warmup-{WARMUP_STEPS}-EPOCH-{EPOCHS}-EMBED-{EMBED_DIM}-FFN-{FFN_DIM}-DR-{DROPOUT}-NH-{NUM_HEADS}-NL-{NUM_LAYERS}{src_mask_type}{tgt_mask_type}{memory_mask_type}"
    ...
    val_text_tokens, val_unit_tokens, val_alignment = load_data([24], is_val=True)  # Example chunks
    ...
    if CONTINUE_TRAINING_PATH:
        ...
        model.load_state_dict(parse_state_dict(state_dict))

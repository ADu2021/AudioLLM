...
MEMORY_MASK_TYPE = "independent" # none, independent or causal
...
def greedy_decode(model, src, src_mask, max_len, start_symbol, sep_token, answer, known_alignment):
    ...
    for i in range(len(src[0])):
        ...
        for j in range(max_len-1):
            ...
            if known_alignment[0][-1] == (alignment_lens[0][-1] + 1):
                break
...
def demo(text_tokens, unit_tokens, model_name, id):
    ...
    non_sep_mask = (unit_tokens < EOS_TOKEN)
    unit_tokens = unit_tokens[non_sep_mask]
...
def main_inference():
    ...
    for i in range(10):
        ...
        known_alignment = alignment[i:i+1]


        src_mask = generate_square_subsequent_mask(src.size(1))
        
        start_symbol = SOS_TOKEN # We use sep token as start symbol
        sep_token = EOS_TOKEN
        
        # import ipdb; ipdb.set_trace();
        predicted_tokens = greedy_decode(model, src, src_mask, max_len=1000, start_symbol=start_symbol, sep_token=sep_token, answer=answer, known_alignment=known_alignment)

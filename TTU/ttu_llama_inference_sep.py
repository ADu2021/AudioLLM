...
MEMORY_MASK_TYPE = "lookbehind-0" # independent or causal or lookbehind-x
...
def load_data(chunks, is_val=False):
    ...
    for i in range(len(text_tokens)):
        if text_tokens[i][-1] in [627, 5380]: # ".\n"
            text_tokens[i] = text_tokens[i][:-1]
            unit_tokens[i] = unit_tokens[i][:-alignment[i][-1]]
            alignment[i] = alignment[i][:-1]
...
def greedy_decode(model, src, src_mask, max_len, start_symbol, sep_token, answer, gt_align):
    dtype = torch.float32
...
def main_inference():
    ...
    # Continue UnitEmbed lookbehind-1 lookbehind-0
    # model_path = "/afs/cs.stanford.edu/u/duyy/data/checkpoints/TTU/ckpt-2024-08-15_11-10-56-LLaMA-Continue-NGPU-4_BS-32_LR-1e-05-Warmup-3000-EPOCH-5-EMBED-1280-FFN-2048-DR-0.1-NH-8-NL-6-lookbehind-1_tgt_mask-lookbehind-0_mem_mask/epoch-4.bin"

    # SEPERATED (Pretrain)
    model_path = "/afs/cs.stanford.edu/u/duyy/data/checkpoints/TTU/ckpt-2024-08-08_22-18-05-LLaMA-UnitEmbed-Seperated-Resample-NGPU-3_BS-64_LR-1e-05-Warmup-3000-EPOCH-6-EMBED-1280-FFN-2048-DR-0.1-NH-8-NL-6-causal_tgt_mask-causal_mem_mask/epoch-4.bin"
    ...
    # for i in range(len(ss)):
        ...
        # import ipdb; ipdb.set_trace();
        for j in range(len(text_tokens[i])):
            src = torch.tensor(text_tokens[i:i+1], dtype=torch.long)[:, j : j+1]
            answer = torch.tensor(unit_tokens[i:i+1], dtype=torch.long)[:, cur_pos : cur_pos+alignment[i][j]].to('cuda')
            gt_align = torch.tensor(alignment[i:i+1], dtype=torch.long)[:, j : j+1]
            cur_pos += alignment[i][j]
            ...
            # import ipdb; ipdb.set_trace();
            # beam_search_decode(model, src, src_mask, max_len, start_symbol, sep_token, beam_size=10)
            predicted_token = greedy_decode(model, src, src_mask, max_len=1000, start_symbol=start_symbol, sep_token=sep_token, answer=answer, gt_align=gt_align)
            predicted_token_list.append(predicted_token)
        predicted_tokens = torch.cat(predicted_token_list, dim=1)

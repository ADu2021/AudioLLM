...
def main():
    ...
    run_name = f"{current_time}-LLaMA-NGPU-{GPU_COUNT}_BS-{BATCH_SIZE}_LR-{LR}-Warmup-{WARMUP_STEPS}-EPOCH-{EPOCHS}-EMBED-{EMBED_DIM}-FFN-{FFN_DIM}-DR-{DROPOUT}-NH-{NUM_HEADS}-NL-{NUM_LAYERS}{src_mask_type}{tgt_mask_type}{memory_mask_type}"
    ...
    # Load real data
    text_tokens, unit_tokens, alignment = load_data(range(1))  # Example chunks
    text_tokens = text_tokens[:32]
    unit_tokens = unit_tokens[:32]
    ...
    scheduler = warmup_scheduler(optimizer, warmup_steps=WARMUP_STEPS)
